...
for dataset in "${datasets[@]}"; do
    for lambda in "${lambdas1[@]}"; do
        echo "Computing $dataset with lambda=$lambda"
        python runners/compute_design_descriptors.py --model-name=lstm --n-generations=100000 --task-name=$dataset --training-strategy=task-arithmetic --lambda_=$lambda 
    done
done
